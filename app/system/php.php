...
trait php  {
    ...
    *   @Mixed
    *   @Param
    *   @ Echo Komutu Güvenlik Protokolü Sadece Db Den Gelen Verilerde Kullanın
    **/
    public static function yazdir($par1){
    $karakter = array('<?php','?>');
    $degistir = array('!Php kodu yasak!','!yasak!');
    $guvenli = str_replace($karakter, $degistir, $par1);
    echo htmlspecialchars($guvenli, ENT_COMPAT,'UTF-8', true);
	}
    ...
    **/
    public static function geridon(){
    $geldigi_sayfa = $_SERVER['HTTP_REFERER'];
    return header("location: $geldigi_sayfa");
	}
    ...
    *   @ Yönlendirme Yapar
    **/
    public static function yonlendir($par){
    return header("location: $par");
	}


	public function sef_link($s){
    $letters = array('ş','Ş','ı','İ','ğ','Ğ','ü','Ü','ö','Ö','ç','Ç','А','Б','В','Г','Д','Е','Ж','З','И','Й','К','Л','М','Н','О','П','Р','С','Т','У','Ф','Х','Ц','Ч','Ш','Щ','Ъ','Ь','Ю','Я','а','б','в','г','д','е','ж','з','и','й','к','л','м','н','о','п','р','с','т','у','ф','х','ц','ч','ш','щ','ъ','ь','ю','я','ي','و','ح','ن','م','ل','گ','ن','ك','ق','ف','غ','ز','ت','د','س','ش','س','ج','ز','ر','ذ','د','خ','ح','ث','ج','ث','ت','پ','ب','ى','ء');
    // Türkçe karakterlerin çevirlecegi karakterler
    $en = array('s','s','i','i','g','g','u','u','o','o','c','c','A','B','V','G','D','E','J','Z','I','Y','K','L','M','N','O','P','R','S','T','U','F','H','C','CH','SH','SHT','A','X','YU','YA','a','b','v','g','d','e','j','z','i','y','k','l','m','n','o','p','r','s','t','u','f','h','c','ch','sh','sht','a','x','yu','ya','y','w','h','n','m','l','g','n','k','q','f','gh','z','t','d','s','sh','s','j','z','r','dh','d','kh','h','th','j','th','t','p','b','a','ha');
    $s = str_replace($letters,$en,$s);
    $s = strtolower($s);
    ...
    *   @ Açılış Hızı Testi
    **/
	public static function issetpg($par){
...
*   @ Link Bölme İşlemi
**/
public static function linkayrac($sayi){
	$linkdizi = null;
	$sayi = null;
$linkdizi = explode("/",$_SERVER['REQUEST_URI']);
return $linkdizi[$sayi];
}
    ...
    **/
	public static function timecover($zaman){
    ...
    */
	public static function files($par1,$par2){ //yeni güncelleme

   	return $_FILES[$par1][$par2];
    ...
    */
	...
	public static function guvenliget($par){

	return  htmlspecialchars(htmlentities(addslashes($par)));

	}

    public static function guvenlipost($par){
      $karakter = array('<?php','?>');
      $degistir = array('Php Kodu Yasak','Yasak');
      $par = str_replace($karakter, $degistir, $par);

      $karakter2 = array('<script>','</script>');
      $degistir2 = array('Javascript Kodu Yasak','Yasak');
      $par = str_replace($karakter2, $degistir2, $par);
    ...
    **/
	public static function isinfo($str, $start, $end){
    ...
    *	Boşlukları Siler
    **/
	public static function bosluksil($string){
    ...
    *   @Param
    *   @ Önemli Bilgi Gizleme
    *	Ck Editor Boşlukları Siler
    **/
	public static function ckbosluksil($string){
        $string = htmlspecialchars_decode($string);
        $string = preg_replace('/\s+/',' ', $string);
        $string = trim($string);
        return $string;
    ...
    **/
	public static function location($par,$sure){

		echo  "<meta http-equiv='refresh' content='".$sure.";URL=".$par."'>";
